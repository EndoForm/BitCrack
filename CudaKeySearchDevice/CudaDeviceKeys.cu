...
uint64_t CudaDeviceKeys::getIndex(int block, int thread, int idx) {
  // Total number of threads
  uint64_t totalThreads = (uint64_t)_blocks * _threads;

  uint64_t base = (uint64_t)idx * totalThreads;

  // Global ID of the current thread
  uint64_t threadId = (uint64_t)block * _threads + thread;

  return base + threadId;
}

void CudaDeviceKeys::splatBigInt(unsigned int *dest, int block, int thread,
  ...
  uint64_t base = (uint64_t)idx * _blocks * _threads * 8;

  uint64_t index = base + threadId;
...
secp256k1::uint256 CudaDeviceKeys::readBigInt(unsigned int *src, int block,
                                              int thread, int idx) {
  unsigned int value[8] = {0};
...
cudaError_t CudaDeviceKeys::allocateChainBuf(unsigned int count) {
  cudaError_t err =
      cudaMalloc(&_devChain, (size_t)count * sizeof(unsigned int) * 8);
...
cudaError_t CudaDeviceKeys::initializeBasePoints() {
  ...
  cudaError_t err =
      cudaMalloc(&_devBasePointX, sizeof(unsigned int) * count * 8);
  ...
  err = cudaMemcpy(_devBasePointX, tmpX, count * 8 * sizeof(unsigned int),
  ...
  err = cudaMemcpy(_devBasePointY, tmpY, count * 8 * sizeof(unsigned int),
                   cudaMemcpyHostToDevice);
...
cudaError_t
CudaDeviceKeys::init(int blocks, int threads, int pointsPerThread,
  ...
  err = cudaMalloc(&_devPrivate, (size_t)sizeof(unsigned int) * count * 8);
  if (err) {
    return err;
  }

  // Clear private keys
  err = cudaMemset(_devPrivate, 0, (size_t)sizeof(unsigned int) * count * 8);
  if (err) {
    return err;
  }

  err = allocateChainBuf((uint64_t)_threads * _blocks * _pointsPerThread);
  if (err) {
    return err;
  }
  ...
  err = cudaMemcpy(_devPrivate, tmp, (size_t)count * sizeof(unsigned int) * 8,
                   cudaMemcpyHostToDevice);
...
cudaError_t CudaDeviceKeys::doStep() {
  multiplyStepKernel<<<_blocks, _threads>>>(_devPrivate, _pointsPerThread,
                                            _step, _devChain, _devBasePointX,
                                            _devBasePointY);
...
bool CudaDeviceKeys::selfTest(
  ...
  cudaError_t err =
      cudaMemcpy(xBuf, _devX, sizeof(unsigned int) * 8 * numPoints,
                 cudaMemcpyDeviceToHost);

  err = cudaMemcpy(yBuf, _devY, sizeof(unsigned int) * 8 * numPoints,
                   cudaMemcpyDeviceToHost);
  ...
  for (int block = 0; block < _blocks; block++) {
    for (int thread = 0; thread < _threads; thread++) {
      for (int idx = 0; idx < _pointsPerThread; idx++) {
        ...
        secp256k1::ecpoint p2 =
            secp256k1::multiplyPoint(privateKey, secp256k1::G());
