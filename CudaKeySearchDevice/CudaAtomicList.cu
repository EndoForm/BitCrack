...
__device__ void atomicListAdd(void *info, unsigned int size) {
  ...
  unsigned char *ptr = (unsigned char *)(_LIST_BUF[0]) + (size_t)count * size;
...
cudaError_t CudaAtomicList::init(unsigned int itemSize, unsigned int maxItems) {
  ...
  _countHostPtr = NULL;
  cudaError_t err =
      cudaHostAlloc(&_countHostPtr, sizeof(unsigned int), cudaHostAllocMapped);
  if (err) {
    goto end;
  }
...
unsigned int CudaAtomicList::size() { return *_countHostPtr; }

void CudaAtomicList::clear() { *_countHostPtr = 0; }
