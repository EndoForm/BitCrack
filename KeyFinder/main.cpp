...
DeviceParameters getDefaultParameters(const DeviceManager::DeviceInfo &device) {
  ...
  // Scale blocks with compute units (SMs).
  // For L4 (60 SMs), this gives 240 blocks, ensuring high occupancy.
  if (device.computeUnits > 0) {
    p.blocks = device.computeUnits * 4;
  } else {
    p.blocks = 32;
  }

  // Modern GPUs with large VRAM (like L4's 24GB) benefit from larger batches
...
static KeySearchDevice *getDeviceContext(DeviceManager::DeviceInfo &device,
  ...
  if (device.type == DeviceManager::DeviceType::OpenCL) {
    return new CLKeySearchDevice(device.physicalId, threads, pointsPerThread,
                                 blocks);
  }
#endif
